...
BOFT v1 for FLUX.2 Klein uses the same training script with a different network module:
...
accelerate launch --num_cpu_threads_per_process 1 --mixed_precision bf16 src/musubi_tuner/flux_2_train_network.py \
    ...
    --optimizer_type adamw8bit --learning_rate 1e-4 --gradient_checkpointing \
    --max_data_loader_n_workers 2 --persistent_data_loader_workers \
    --network_module networks.boft_flux_2 --network_dim 32 --network_alpha 0.1 \
    --network_args "rescaled=True" \
    --max_train_epochs 16 --save_every_n_epochs 1 --seed 42 \
    --output_dir path/to/output_dir --output_name name-of-boft
```

- Uses `flux_2_train_network.py`.
- **Requires** specifying `--vae` (not `--ae`), `--text_encoder`
- **Requires** specifying `--network_module networks.lora_flux_2` for LoRA, or `--network_module networks.boft_flux_2` for BOFT v1.
- `--mixed_precision bf16` is recommended for FLUX.2 training.
- `--timestep_sampling flux2_shift` is recommended for FLUX.2.
- Use the `--model_version` option for Flux.2 Klein training (if omitted, defaults to `dev`).
- BOFT v1 is available only for Klein model versions (`klein-4b`, `klein-9b`, `klein-base-4b`, `klein-base-9b`) via `--network_module networks.boft_flux_2`.
- In BOFT v1, `network_dim` controls the BOFT structure, `network_alpha` is exported as BOFT `alpha`, and `--network_args "rescaled=True"` enables the optional BOFT rescale tensor.
...
- LoRAの場合は`--network_module networks.lora_flux_2`、BOFT v1の場合は`--network_module networks.boft_flux_2`を指定する必要があります。
...
Exported BOFT weights can be passed through `--lora_weight` in the same way for FLUX.2 Klein model versions. Native BOFT inference in this implementation is limited to Klein variants.
